...
class SQLiteDatabase:
    ...
    # updates the number of connection errors (in a row)
    def update_connection_errors(self, user_id, connection_errors):
        with self.connection:
            sql = ''' UPDATE `users`
                      SET `connection_errors` = ? 
                      WHERE `user_id` = ?'''
            return self.cursor.execute(sql, (connection_errors, user_id))
