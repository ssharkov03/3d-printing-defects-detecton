        ...
        for user in users:
            ...
            if ret and frame is not None:
                ...
                if user[8]:
                    ...
                    # if we want to get all notifications, then we need check image for defects every T seconds
                    ...
                    # if only scheduled notifications needed, then get predictions only when needed time is gone
                    if user[5] * T >= user[4]:
                        verdict, prob_yes_defects = get_prediction(frame)
                        # load photo to memory and push it to bot
                        bio = BytesIO()
